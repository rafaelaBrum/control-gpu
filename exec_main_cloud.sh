...
echo " ----------------------------
 50 epochs
 ---------------------------- "
for instance_type in g3.8xlarge
...
do
  ...
  while [  $COUNTER -lt 4 ]; do
   ...
   ----------------------------"
   FOLDER="centralized_2_epoch_$((COUNTER))_folder_${instance_type}_spot"
    echo python3 test_CNN_TIL_without_fl_cloud.py --folder $FOLDER --instance_type $instance_type --epochs 2
    python3  test_CNN_TIL_without_fl_cloud.py --folder $FOLDER --instance_type $instance_type --epochs 2
    sleep 30m
    COUNTER=$((COUNTER+1))
  done
done
