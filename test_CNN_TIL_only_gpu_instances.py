...
def main():
    ...
    while vm_server.state not in (CloudManager.TERMINATED, None):
        ...
        if has_command_finished(vm_server, 'test_server'):
            logging.info('Server has finished execution!')
            finish_vm(vm_server, os.path.join(args.folder, 'server'), 'screen_log', n_parties)
        time.sleep(5)
...
def controlling_client_flower_from_server(vm: VirtualMachine,
    ...
    test_folder = f'{folder}/CellRep/{n_parties}_clients/{client_id}/testset'

    __prepare_vm_client(vm, "localhost:8080", client_id, train_folder, test_folder, epochs)
    logging.info(f"Client_{client_id} created!")
